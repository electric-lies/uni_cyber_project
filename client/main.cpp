...
#define SAFE_UNSIGNED(n) (n & 0xffffffff)
...
std::vector<std::string> splitString(const std::string &str, int chunkSize)
{
    ...
    {
        int endPos = std::min(startPos + chunkSize, length);
        chunks.push_back(str.substr(startPos, endPos - startPos));
        startPos = endPos;
    }
...
unsigned long memcrc2(char *b, size_t n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        s = SAFE_UNSIGNED((s << 8)) ^ crctab[0][tabidx];
    }
    ...
    while (n)
    {
        c = n & 0377;
        n = n >> 8;
        s = SAFE_UNSIGNED(s << 8) ^ crctab[0][(s >> 24) ^ c];
    }
    return (unsigned long)SAFE_UNSIGNED(~s);
}

unsigned long chunk_crc(std::string content)
{
    size_t size = content.size();
    char *b = new char[size];
    std::memcpy(b, content.c_str(), size);

    return memcrc2(b, size);
}
...
// Helper function to append data to the buffer
template <typename T>
void append_to_buffer(std::vector<uint8_t> &buffer, T value)
{
...
std::pair<uint16_t, std::vector<char>> parseServerResponse(boost::asio::ip::tcp::socket &socket)
{
    boost::system::error_code ec;

    // Reading the fixed-size header first
    std::vector<char> header(7); // 1 byte version, 2 bytes answer code, 4 bytes payload length
    size_t length = boost::asio::read(socket, boost::asio::buffer(header), ec);
    ...
    uint8_t version = header[offset++];

    // Read the answer code (2 bytes)
    uint16_t answerCode;
    memcpy(&answerCode, &header[offset], sizeof(answerCode));
    offset += sizeof(answerCode);

    // Read the payload length (4 bytes)
    uint32_t payloadLength;
    memcpy(&payloadLength, &header[offset], sizeof(payloadLength));

    std::cout << "[x] "
              << "Version: " << static_cast<int>(version) << std::endl;
    std::cout << "[x] "
              << "Answer Code: " << answerCode << std::endl;
    std::cout << "[x] "
              << "Payload Length: " << payloadLength << std::endl;
...
// Function to write user_name, client_id (in hex representation), and encoded private key to a file
void writeToFile(const std::string &user_name, const std::vector<char> &client_id, const RSAPrivateWrapper &rsapriv)
{
    ...
    // Write user_name to the first line
    file << user_name << std::endl;

    // Write client_id (in hex representation) to the second line
    // ffffff
    std::stringstream ss;
    ...
    auto result = ss.str();
    std::string pattern = "ffffff";
    removeSubstrs(result, pattern);
    file << result;
    file << std::endl;

    // Encode and write the private key to the third line
    std::string encoded_private_key = Base64Wrapper::encode(rsapriv.getPrivateKey());
    encoded_private_key.erase(std::remove(encoded_private_key.begin(), encoded_private_key.end(), '\n'), encoded_private_key.cend());
    file << encoded_private_key << std::endl;

    file.close();
}

std::string read_all_file(std::string_view path)
{
...
// Function to read user_name, client_id (in hex representation), and encoded private key from a file
std::tuple<bool, std::string, std::vector<char>, std::string> readFromFile()
{
    std::ifstream file("me.info");
    if (!file.is_open())
    {
        std::cerr << "[x] Error opening file "
                  << "me.info" << std::endl;
        return std::make_tuple(false, "", std::vector<char>(), "");
    }

    std::cout << "[x] opening file "
              << "me.info" << std::endl;
    ...
    for (size_t i = 0; i < hex_client_id.length(); i += 2)
    {
        std::string byteString = hex_client_id.substr(i, 2);
    ...
    return std::make_tuple(true, user_name, client_id, encoded_private_key);
}
...
int main()
{
    ...
    std::vector<char> client_id;
    std::vector<char> encrypted_private_key_with_client_id;
    std::string address_port;
    // int port;

    // Read address and port from the first line
    std::getline(file, address_port);
    if (address_port.empty())
    {
        std::cerr << "Failed to read address" << std::endl;
        return 1;
    }
    ...
    if (user_name.empty())
    {
        std::cerr << "Failed to read user_name" << std::endl;
        return 1;
    }

    std::string filename;
    // Read user ID from the second line
    ...
    std::string temp_user_id = "0123456789123456";
    std::string address;
    std::string port;

    address = address_port.substr(0, address_port.find(":"));
    port = address_port.substr(address_port.find(":") + 1, address_port.length());

    RSAPrivateWrapper rsapriv;
    std::string private_key;

    auto me_info = readFromFile();
    // cant read me.info, probably because dont exit
    if (!std::get<0>(me_info))
    {
        private_key = rsapriv.getPrivateKey();
        // Prepare the binary message
        std::vector<uint8_t> message;
        message.reserve(16 + 1 + 2 + 4 + 255); // Allocate enough space for the entire message

        // Append client ID (padded or truncated to 16 bytes)
        message.insert(message.end(), temp_user_id.begin(), temp_user_id.begin() + std::min(temp_user_id.size(), size_t(16)));
        if (temp_user_id.size() < 16)
        {
            message.insert(message.end(), 16 - temp_user_id.size(), 0);
        }

        // Append other integers
        append_to_buffer(message, uint8_t(3));     // 1 byte for the number 3
        append_to_buffer(message, uint16_t(1025)); // 2 bytes for the number 1025
        append_to_buffer(message, uint32_t(255));  // 4 bytes for the number 255

        // Append user_name (padded or truncated to 255 bytes)
        message.insert(message.end(), user_name.begin(), user_name.begin() + std::min(user_name.size(), size_t(255)));
        if (user_name.size() < 255)
        {
            message.insert(message.end(), 255 - user_name.size(), 0);
        }
        ...
        try
        {
            ...
            std::cout << "[x] "
                      << "sending: " << boost::asio::buffer(message).data() << "\n";
            // Send user ID and payload to the server
            boost::asio::write(socket, boost::asio::buffer(message));

            // Receive data from the server

            std::pair<uint16_t, std::vector<char>> p;
            p = parseServerResponse(socket);
            ...
            {
                std::cout << "[x] "
                          << "registration succeeded, received clientID" << std::endl;
            }

            else if (response_code == 1601)
            {
                std::cout << "[x] "
            ...
            {
                std::cout << "WTF is this response? " << response_code << std::endl;
            }
        }
        ...
        writeToFile(user_name, client_id, rsapriv);

        // 2. get the public key
        std::string pubkey = rsapriv.getPublicKey();
        std::cout << "[x] "
                  << "publickey is " << std::endl;
        hexify(std::vector<unsigned char>(pubkey.data(), pubkey.data() + pubkey.length() + 1).data(), pubkey.length());
        std::vector<uint8_t> message2;
        message2.reserve(16 + 1 + 2 + 4 + 255 + 160); // Allocate enough space for the entire message2

        // Append client ID (padded or truncated to 16 bytes)
        message2.insert(message2.end(), client_id.begin(), client_id.begin() + std::min(client_id.size(), size_t(16)));
        if (client_id.size() < 16)
        {
            message2.insert(message2.end(), 16 - client_id.size(), 0);
        }

        // Append other integers
        append_to_buffer(message2, uint8_t(3));
        append_to_buffer(message2, uint16_t(1026));
        append_to_buffer(message2, uint32_t(255 + 160));

        // Append user_name (padded or truncated to 255 bytes)
        message2.insert(message2.end(), user_name.begin(), user_name.begin() + std::min(user_name.size(), size_t(255)));
        if (user_name.size() < 255)
        {
            message2.insert(message2.end(), 255 - user_name.size(), 0);
        }

        message2.insert(message2.end(), pubkey.begin(), pubkey.begin() + std::min(pubkey.size(), size_t(255)));
        if (pubkey.size() < 255)
        {
            message2.insert(message2.end(), 255 - pubkey.size(), 0);
        }
        ...
        try
        {
            ...
            std::cout << "[x] "
                      << "sending: " << boost::asio::buffer(message2).data() << "\n";
            // Send user ID and payload to the server
            ...
            p = parseServerResponse(socket);
            response_code = p.first;
            encrypted_private_key_with_client_id = p.second;

            if (response_code == 1602)
            {
                std::cout << "[x] "
                          << "key swap succeeded, received private key" << std::endl;
            }

            else
            {
                std::cout << "[x] "
                          << "WTF is this response? " << response_code << std::endl;
            }
        }
        catch (std::exception &e)
        {
    ...
    else
    {
        ...
        std::string user_name = std::get<1>(me_info);
        client_id = std::get<2>(me_info);
        private_key = Base64Wrapper::decode(std::get<3>(me_info));

        std::vector<uint8_t> message;
        message.reserve(16 + 1 + 2 + 4 + 255); // Allocate enough space for the entire message

        // Append client ID (padded or truncated to 16 bytes)
        message.insert(message.end(), client_id.begin(), client_id.begin() + std::min(client_id.size(), size_t(16)));
        if (client_id.size() < 16)
        {
            message.insert(message.end(), 16 - client_id.size(), 0);
        }

        // Append other integers
        append_to_buffer(message, uint8_t(3));     // 1 byte for the number 3
        append_to_buffer(message, uint16_t(1027)); // 2 bytes for the number 1025
        append_to_buffer(message, uint32_t(255));  // 4 bytes for the number 255
        ...
        try
        {
            ...
            {
                std::cout << "[x] "
                          << "reconnection succeeded, received AES key" << std::endl;
            }
    ...
    std::string enc_aes_key = std::string(encrypted_private_key_with_client_id.data(), encrypted_private_key_with_client_id.size()).substr(16);
    std::cout << "[x] "
              << "aes key received is " << std::endl;
    hexify(std::vector<unsigned char>(enc_aes_key.data(), enc_aes_key.data() + enc_aes_key.length() + 1).data(), enc_aes_key.length());

    RSAPrivateWrapper decrypt = RSAPrivateWrapper(private_key);

    std::string aes_key = decrypt.decrypt(enc_aes_key);
    std::cout << "[x] key after RSA decrypting is " << std::endl;
    hexify(std::vector<unsigned char>(aes_key.data(), aes_key.data() + aes_key.length() + 1).data(), aes_key.length());

    std::vector<unsigned char> key = std::vector<unsigned char>(aes_key.data(), aes_key.data() + aes_key.length() + 1);

    AESWrapper aes(key.data(), AESWrapper::DEFAULT_KEYLENGTH);
    hexify(aes.getKey(), 16);

    constexpr auto read_size = std::size_t(16);

    std::string buf = read_all_file(filename);

    int chunkSize = 16000;

    std::vector<std::string> chunks = splitString(buf, chunkSize);
    ...
    for (const auto &chunk : chunks)
    {
        ...
        auto encrypted_chunk = aes.encrypt(chunk.c_str(), chunk.length());

        message3.reserve(23 + 4 + 4 + 4 + 255 + encrypted_chunk.size()); // Allocate enough space for the entire message3

        // Append client ID (padded or truncated to 16 bytes)
        message3.insert(message3.end(), client_id.begin(), client_id.begin() + std::min(client_id.size(), size_t(16)));
        if (client_id.size() < 16)
        {
            message3.insert(message3.end(), 16 - client_id.size(), 0);
        }

        // Append other integers
        append_to_buffer(message3, uint8_t(3));
        append_to_buffer(message3, uint16_t(1028));
        append_to_buffer(message3, uint32_t(4 + 4 + 4 + 255 + encrypted_chunk.size()));
        append_to_buffer(message3, uint32_t(encrypted_chunk.size()));
        std::cout << "[x] "
                  << "encrypted_chunk filesize is " << encrypted_chunk.size() << "\n";
        append_to_buffer(message3, uint32_t(std::filesystem::file_size(filename)));
        append_to_buffer(message3, uint16_t(packet_count));
        append_to_buffer(message3, uint16_t(chunks.size()));
        message3.insert(message3.end(), filename.begin(), filename.begin() + std::min(filename.size(), size_t(255)));
        if (filename.size() < 255)
        {
            message3.insert(message3.end(), 255 - filename.size(), 0);
        }
        // append_to_buffer(message3, filename);

        message3.insert(message3.end(), encrypted_chunk.begin(), encrypted_chunk.begin() + encrypted_chunk.size());
        unsigned long crc_from_server = 0;
        try
        {
            boost::asio::io_context io_context;
            std::cout << "[x] "
            ...
            std::cout << "[x] "
                      << "sending: " << message3.size() << " bytes \n";
            // Send user ID and payload to the server
            boost::asio::write(socket, boost::asio::buffer(message3));
            ...
            for (int i = 0; i < 4; i++)
            {
                crc_from_server |= ((uint32_t)((uint8_t)(p.second[p.second.size() - 4 + i]))) << (i * 8);
                std::cout << std::to_string((uint8_t)p.second[p.second.size() - 4 + i]) << " -> "
                          << std::to_string(((uint32_t)((uint8_t)(p.second[p.second.size() - 4 + i]))) << (i * 8)) << "+="
                          << std::to_string(crc_from_server) << std::endl;
            }
            // crc_from_server = ((uint32_t)p.second[275] << 24) + ((uint32_t)p.second[276] << 16) + ((uint32_t)p.second[277] << 8) + p.second[278];
            // std::cout << "[x] CRC from server: " << ((uint32_t)p.second[275] << 24) << ", " << ((uint32_t)p.second[276] << 16) << ", " << ((uint32_t)p.second[277] << 8) << ", " << ((uint32_t)p.second[278]) << std::endl;

            // for (size_t i = p.second.size() - 4; i < p.second.size(); ++i)
            // {
            //     crc_from_server += std::to_string(p.second[i]);
            // }
            std::cout
                << "[x] "
                << "checksum is " << crc_from_server << std::endl;
            // TODO: figure out why file transfer is broken and how to checksum
            if (response_code == 1603)
            {
                std::cout << "[x] "
                          << "file moved completely, check sum test is due" << std::endl;
            }
        ...
        uint16_t status_code_of_crc;
        auto calculated_crc = chunk_crc(chunk);

        if (crc_from_server == calculated_crc)
        {
            status_code_of_crc = 1029;
            std::cout << "[x] crces are equal "
                      << "\n";
        }
        else
        {
            status_code_of_crc = 1030;
            // hexify(std::vector<unsigned char>(calculated_crc.data(), calculated_crc.data() + calculated_crc.length() + 1).data(), calculated_crc.length());
            // hexify(std::vector<unsigned char>(crc_from_server.data(), crc_from_server.data() + crc_from_server.length() + 1).data(), crc_from_server.length());
            std::cout << "[x] " << crc_from_server << "!=" << calculated_crc << std::endl;
            std::cout << "[x] crces are different, retrying "
                      << "\n";
        }

        std::vector<uint8_t> message4;
        message4.reserve(16 + 1 + 2 + 4 + 255); // Allocate enough space for the entire message2
        // Append client ID (padded or truncated to 16 bytes)
        message4.insert(message4.end(), client_id.begin(), client_id.begin() + std::min(client_id.size(), size_t(16)));
        if (client_id.size() < 16)
        {
            message4.insert(message4.end(), 16 - client_id.size(), 0);
        }
        append_to_buffer(message4, uint8_t(3));
        append_to_buffer(message4, status_code_of_crc);
        append_to_buffer(message4, uint32_t(255));
        message4.insert(message4.end(), filename.begin(), filename.begin() + std::min(filename.size(), size_t(255)));
        if (filename.size() < 255)
        {
            message4.insert(message4.end(), 255 - filename.size(), 0);
        }
        ...
        try
        {
            ...
            std::cout << "[x] "
                      << "sending: " << message4.size() << " bytes \n";
            // Send user ID and payload to the server
            boost::asio::write(socket, boost::asio::buffer(message4));
            ...
            {
                std::cout << "[x] "
                          << "server accepted file after crc verification, process done" << std::endl;
            }
