...
unsigned long readfile2(std::string fname)
{
...
int main()
{
    ...
    RSAPrivateWrapper decrypt = RSAPrivateWrapper(rsapriv.getPrivateKey());
    ...
    std::ifstream content_file(filename);
    ...
    while (content_file.read(&buf[0], read_size))
    {
        std::cout << "reading & encrypting" << std::endl;
        std::string cipher = aes.encrypt(buf.c_str(), buf.length());
        encrypted.append(cipher, 0, content_file.gcount());
    }
    // std::string cipher = aes.encrypt(buf.c_str(), buf.length());
    // encrypted.append(cipher, 0, content_file.gcount());

    std::cout << "encrypted content: " << std::endl;
    hexify(std::vector<unsigned char>(encrypted.data(), encrypted.data() + encrypted.length() + 1).data(), encrypted.length());

    std::vector<uint8_t> message3;
    message3.reserve(23 + 4 + 4 + 4 + 255 + encrypted.size()); // Allocate enough space for the entire message3
    ...
    append_to_buffer(message3, uint32_t(4 + 4 + 4 + 255 + encrypted.size()));
    append_to_buffer(message3, uint32_t(encrypted.size()));
    std::cout << "[x] "
              << "encrypted filesize is " << encrypted.size() << "\n";
    append_to_buffer(message3, uint32_t(std::filesystem::file_size(filename)));
    append_to_buffer(message3, uint16_t(1));
    append_to_buffer(message3, uint16_t(1));
    ...
    message3.insert(message3.end(), encrypted.begin(), encrypted.begin() + encrypted.size());
    ...
    auto calculated_crc = readfile2(filename);
